...
export async function POST(req: NextRequest, res: NextResponse) {
    try {
        ...
        const id = requestBody.appointmentId.toUpperCase();
        const clientId = requestBody.clientId
        const isDelete = requestBody.isDelete; 
    

        const response = await axios.post(`http://localhost:5009/appointments/${id}/${isDelete ? "delete" : "update"}`, {
            "clientId" : clientId,
            "ScheduledTime": requestBody.appointmentDate,
        } );
